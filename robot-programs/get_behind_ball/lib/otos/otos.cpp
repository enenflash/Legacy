...
Vector OTOS::get_posv() {
    ...
    this->total_x = this->total_x + abs(this->previous_x - position.x * 100);
    this->previous_x = position.x * 100;
    return Vector(position.x * 100, position.y *100); //position.y * 1.15 * 100 + this->total_y/137
}