...
public:
  ...
  void moveTo(int TARGET_X, int TARGET_Y, int BUFFER_RANGE) {
    int distance = sqrt(pow((TARGET_X-ps.x), 2)+pow((TARGET_Y-ps.y),2));
  ...
  // function for robot to defend the goal
  void defendGoal() {
    moveTo(91, 160, 5);
  }
  ...
  void getOffset() {
    ...
    float x_from_center = abs(FIELD_WIDTH/2.0 - ps.x);
    float y_from_goal = ps.y - 30;
    ...
    // if (ps.x < FIELD_WIDTH/2.0 && direction == 12 && strength > 35) {
    //   //offset = 25;
    //   offset = -(1/3.0) * ps.x + 35;
    // }
    // else if (ps.x > FIELD_WIDTH/2.0 && direction == 12 && strength > 35) {
    //   //offset = -25;
    //   offset = (-(1/3.0) * (FIELD_WIDTH - ps.x) + 35) * -1;
    // }
    // else {
    //   offset = 0;
    // }
    //abs(offset) < 6 || 
    if (direction != 12 || strength < 35) {
      offset = 0;
    }
